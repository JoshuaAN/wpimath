...
class WPILIB_DLLEXPORT Problem {
 public:
  ...
  void SubjectTo(
    ...
    for (int row = 0; row < Rows; ++row) {
      for (int col = 0; col < Cols; ++col) {
        Variable<1, 1> slack = Var();
        m_slackIndexes.emplace_back(m_leaves.size() - 1);
        m_equalityConstraints(oldSize + row * Cols + col) =
            std::move(constraint(row, col).variable.GetAutodiff() + slack.GetStorage()(0,0).GetAutodiff());
      }
    }
  }
  ...
  Eigen::MatrixXd ModifiedCholeskyFactorization(const Eigen::Ref<Eigen::MatrixXd>& H);
...
private:
  ...
  // Cost function: f(x)
  std::optional<autodiff::var> m_f;

  // Equality constraints: c(x) = 0
  autodiff::VectorXvar m_equalityConstraints;

  // Slack variable indexes: s ≥ 0
  std::vector<int> m_slackIndexes;
